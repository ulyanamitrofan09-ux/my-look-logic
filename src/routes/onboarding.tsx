...
const COLORS = [
  { id: "warm", label: "Тёплый", desc: "Золотистая/оливковая кожа, карие или зелёные глаза", grad: "linear-gradient(135deg,#E8C39E,#B97A4A)" },
  { id: "cool", label: "Холодный", desc: "Розоватая/фарфоровая кожа, серые или голубые глаза", grad: "linear-gradient(135deg,#E9D4DA,#7390B0)" },
  { id: "neutral", label: "Нейтральный", desc: "Сочетание обоих", grad: "linear-gradient(135deg,#E8C39E,#7390B0)" },
];
const BODIES = ["Прямоугольник", "Песочные часы", "Треугольник", "Перевёрнутый треугольник", "Овал"];
const VIBES = ["Минимализм", "Классика", "Тренды", "Романтика", "Деловой", "Кэжуал"];
...
function Onboarding() {
  ...
  const finish = async () => {
    ...
    toast.success("Профиль стиля сохранён");
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-[428px] px-6 py-8">
        ...
        {step === 0 && (
          <>
            <h2 className="font-serif text-3xl mb-2">Ваш цветотип?</h2>
            <p className="text-muted-foreground mb-6">Это определит, какие цвета подчеркнут вашу внешность.</p>
        ...
        {step === 1 && (
          <>
            <h2 className="font-serif text-3xl mb-2">Ваш силуэт?</h2>
            <p className="text-muted-foreground mb-6">Для сбалансированных пропорций.</p>
        ...
        {step === 2 && (
          <>
            <h2 className="font-serif text-3xl mb-2">Ваш стиль?</h2>
            <p className="text-muted-foreground mb-6">Выберите до 3.</p>
            <div className="grid grid-cols-2 gap-3">
              {VIBES.map(v => (
        ...
        <div className="mt-10 flex gap-3">
          {step > 0 && <button onClick={() => setStep(step - 1)} className="btn-outline flex-1">Назад</button>}
          {step < 2 ? (
            <button disabled={!canNext} onClick={() => setStep(step + 1)} className="btn-primary flex-1">Далее</button>
          ) : (
            <button disabled={!canNext || busy} onClick={finish} className="btn-primary flex-1">{busy ? "…" : "Готово"}</button>
          )}
        </div>
      </div>
    </div>
